...
if (!isset($_REQUEST['test_limit_time']) OR (empty($_REQUEST['test_limit_time']))) {
	$test_limit_time = 1;
} else {
	$test_limit_time = intval($_REQUEST['test_limit_time']);
}
	...
	case 'update':{ // Update
		...
		if ($formstatus = F_check_form_fields()) {
			...
			$sql = 'UPDATE '.K_TABLE_TESTS.' SET
                ...
                test_limit_time=\''.$test_limit_time.'\',
		...
		if (!isset($test_id) OR empty($test_id)) {
			...
			if ($r = F_db_query($sql, $db)) {
				if ($m = F_db_fetch_array($r)) {
					...
					$test_limit_time = $m['test_limit_time'];
...
echo getFormRowTextInput('test_limit_time', $l['w_limit_time'], $l['h_limit_time'], '', $test_limit_time, '^([0-9\+\-]*)$', 20, false, false, false);
