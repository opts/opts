 ...
 */
function O_testlimittime($test_id) {
    global $db;
    $limit_sql = 'SELECT test_limit_time FROM tce_tests WHERE test_id = \'' . $test_id . '\'';
    $tlt_result = mysqli_query($db, $limit_sql);
    if ($tlt_result != false) {
        $r = mysqli_fetch_array($tlt_result);
        return $r['test_limit_time'];
    }
    return NULL;
}
		...
		while ($m = F_db_fetch_array($r)) { // for each active test
			...
			if (F_isValidTestUser($m['test_id'], $_SESSION['session_user_ip'], $m['test_ip_range'])) {
				...
				if (!$expired) {
                    // fetch tried_time
                    require_once '../../shared/code/tce_functions_test_stats.php';
                    $data = F_getAllUsersTestStat($m['test_id'], 0, $user_id);
                    $limit_time_count = $data['num_records'];

                    // fetch test_limit_time
                    $num_limit_time = O_testlimittime($m['test_id']);
					switch ($test_status) {
						...
						case 3: { // 3 = all questions have been answered
							// continue test
							$str .= '<a href="tce_test_execute.php?testid='.$m['test_id'].'" title="'.$l['h_continue'].'" class="xmlbutton">'.$l['w_continue']. '(第' . $limit_time_count . '次测试)</a>';
							break;
						}
						default: { // 4 or greater = test can be repeated
							if (F_getBoolean($m['test_repeatable'])) {
                                // on override, remove link
                                if ($num_limit_time > $limit_time_count) {
								// print execute test link
								    $str .= '<a href="';
								    if (K_DISPLAY_TEST_DESCRIPTION OR !empty($m['test_password'])) {
									    // display test description before starting
                                ...
                                $str .= '?testid='.$m['test_id'].'&amp;repeat=1" title="'.$l['h_repeat_test'].'" class="buttonblue">';
                                    $str .= $l['w_repeat']. '(' . $limit_time_count . '/' . $num_limit_time . ')</a>';
                                } else {
                                    $str .= '<a class="buttongrey">';
                                    $str .= $l['w_test_over_limit'] . '</a>';
                                }
							}
							break;
						}
...
function F_printTestInfo($test_id, $showip=false) {
	...
	if ($r = F_db_query($sql, $db)) {
		if ($m = F_db_fetch_array($r)) {
			...
			$str .= F_twoColRow($l['w_limit_time'], $l['h_limit_time'], $m['test_limit_time']);
...
function F_questionForm($test_id, $testlog_id, $formname) {
	...
	if ($r = F_db_query($sql, $db)) {
		if ($m = F_db_fetch_array($r)) {
            ...
            // insert $qsel ahead of question
            $str .= '<span id="opts-qsel"></span>';
			...
			if ($m['question_type'] == 3) {
				...
				if (F_getBoolean($m['question_inline_answers'])) {
					...
					$str .= '<ol class="answer" style="list-style-type: upper-alpha">'.K_NEWLINE;
				}
    ...
    // short cut for select
                                $str .= <<<HALOJS
    <script type="text/javascript">

var myTarget = document.body.getElementsByTagName('mfenced');
for (i = 0; i < myTarget.length; i++) {
tmp = myTarget[i].getAttribute("close");
tmp = tmp.replace(/&verbar;/g,"|");
myTarget[i].setAttribute("close", tmp);

tmp = myTarget[i].getAttribute("open");
tmp = tmp.replace(/&verbar;/g,"|");
myTarget[i].setAttribute("open", tmp);

}

    var objMisc = document.getElementsByTagName('input');
    var objSel = [];
    for (var i = 0; i < objMisc.length; i++) {
        if (objMisc[i].name == "answerid") {
            objSel.push(objMisc[i]);
        }
    }
...
HALOJS;
        ...
        var sel = key - 49;
        document.getElementById(objSel[sel].id).click();
    }
    }, false);
    </script>
HALOJS;
...
function F_questionsMenu($testdata, $testuser_id, $testlog_id=0, $disable=false) {
	...
	if (!$question_timer) {
		...
		$navlink .= ' />';
        // useless for opts // button for confirm current question
		//$navlink .= '<input type="submit" name="confirmanswer" id="confirmanswer" value="('.$qsel.') '.$l['w_confirm'].'" />';

	}

    // use javascript to insert $qsel
    $navlink .= '<script type="text/javascript"> var idQsel = window.document.getElementById("opts-qsel"); idQsel.innerHTML = ' . $qsel . ' + ". " ; </script> ';
    // MathJax & config insert
    require_once '../../shared/code/omjax.config.php';
    $navlink .= '<script type="text/javascript" src="/opts/mathjax/MathJax.js?config=MML_HTMLorMML.js&locale=zh-hans" ></script>';
    ...
    //js shortcuts to select option & switch question
    $rstr .= <<<HALOJS
    <script type="text/javascript">
    document.addEventListener("keydown", function (event) {
    var key = event.keyCode || event.which; // alternative to ternary - if there is no keyCode, use which
