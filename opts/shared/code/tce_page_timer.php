...
if (!isset($_REQUEST['examtime'])) {
	...
	if (!empty($_REQUEST['timeout_logout'])) {
		$timeout_logout = 'true';
	} else {
		$timeout_logout = 'false';
	}
...
echo '<div>'.K_NEWLINE;
echo '<img src="../../images/logo_opts.png" /> <br />'.K_NEWLINE;
echo '<input type="text" name="timer" id="timer" value="" maxlength="29" title="'.$l['w_clock_timer'].'" readonly="readonly"/>'.K_NEWLINE;
echo '<br />' . K_NEWLINE;
echo '<input type="text" name="timer" id="timer-clock" value="" maxlength="29" title="'.$l['w_clock_timer'].'" readonly="readonly"/>'.K_NEWLINE;
echo '</div>'.K_NEWLINE;
echo '</form>'.K_NEWLINE;
