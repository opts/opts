...
function FJ_timer() {
	if (enable_countdown) { // --- COUNTDOWN MODE ---
		...
		document.getElementById('timer').value = ''+sign+''+diff_hours+':'+diff_minutes+':'+diff_seconds+' ';
	} else { // --- CLOCK MODE ---
		...
		document.getElementById('timer').value = ''+year+'-'+month+'-'+day;
		document.getElementById('timer-clock').value = hour+':'+minute+':'+second;
	}
	return;
}
